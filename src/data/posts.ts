/*
 * Post data lives here rather than in the page component because two things
 * outside React need it: the prerender script, which bakes per-post meta tags
 * into the built HTML, and the RSS generator. Both run in plain Node at build
 * time and cannot import a .tsx file full of JSX.
 *
 * Keeping it out of Blog.tsx also settles the react-refresh lint warning that
 * file carried - a module exporting both a component and a large data table is
 * exactly what that rule is complaining about.
 *
 * Order matters: the array is rendered as-authored, newest first.
 */
